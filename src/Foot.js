...
import {
	...
	BackHandler, AppState, NetInfo, PermissionsAndroid, ActivityIndicator, Modal
} from 'react-native';
...
export default class foot extends Component {
	constructor(props) {
		super(props);
        let tmpTimeout = null;//延时执行对象
		let isReconnectXMPP = false;//唤起APP是否需要重新链接XMPP标记，网络检测失败30次设为true；
		this.state = {
					...
					XMPP.XMPPLoginAccount(
						...
						(error, event) => {
							if (event == '登录成功') {
								...
								// DeviceEventEmitter.emit('netWorkStatue', netWorkIsConnect);
								// alert('重连失败，建议退出重试');
								// this.refs.toast.show('重连失败，建议退出重试', DURATION.LENGTH_SHORT);
	...
	componentDidMount() {
		...
		//监听网络状态
		if (ListenerUtil["Foot_NetListener"]) {
			ListenerUtil["Foot_NetListener"].remove();
		}
		ListenerUtil["Foot_NetListener"] = NetInfo.isConnected.addEventListener(
			'connectionChange',
			(data) => {
		...
		if (Platform.OS == 'android') {
			if (ListenerUtil["Foot_XmppdisconnectListener"]) {ListenerUtil["Foot_XmppdisconnectListener"].remove();}
			ListenerUtil["Foot_XmppdisconnectListener"] = XMPP.on('disconnect', (exception) => {
                connentStatus = false;
                tmpFlag = false;
                this.netWorkPing();//判断网络状况
			});
			if (ListenerUtil["Foot_XmppConnectListener"]) {
				ListenerUtil["Foot_XmppConnectListener"].remove();
			}
			ListenerUtil["Foot_XmppConnectListener"] = XMPP.on('connect', (message) => {
				// DeviceEventEmitter.emit('headTitle',"消息（获取中...）");
				// console.log(message);
			})
			/**
			 * XMPP异常监听
			 */
			if (ListenerUtil["Foot_XmppErrorListener"]) {
				ListenerUtil["Foot_XmppErrorListener"].remove();
			}
			ListenerUtil['Foot_XmppErrorListener'] = XMPP.on('error', (exception) => {
                connentStatus = false;
                tmpFlag = false;
			});
			/**
			 * XMPP登录失败监听
			 */
			if (ListenerUtil['Foot_XmpploginErrorListener']) {
				ListenerUtil['Foot_XmpploginErrorListener'].remove();
			}
			ListenerUtil['Foot_XmpploginErrorListener'] = XMPP.on('loginError', (message) => {
                connentStatus = false;
                tmpFlag = false;
			});
			/**
			 * XMPP普通消息监听
			 */
			if (ListenerUtil['Foot_XmppMessageListener']) {
				ListenerUtil['Foot_XmppMessageListener']
			}
			ListenerUtil['Foot_XmppMessageListener'] = XMPP.on('message', (message) => this._loadMessageToDB(message));
			/**
			 * XMPP登录成功监听
			 */
			if (ListenerUtil['Foot_XmpploginListener']) {
				ListenerUtil['Foot_XmpploginListener'].remove();
			}
			ListenerUtil['Foot_XmpploginListener'] = XMPP.on('login', (message) => {
				// console.log("XMPP连接成功");
                this.isReconnectXMPP = false;
			...
			if (ListenerUtil['Foot_XmppPresenceListener']) {
				ListenerUtil['Foot_XmppPresenceListener'].remove();
			}
			ListenerUtil['Foot_XmppPresenceListener'] = XMPP.on('presence', (message) => {
		...
		this.resetEvent = DeviceEventEmitter.addListener('resetTabNum', () => {
			// //console.log('进入message页刷新监听:' + params);
			this.resetTabNum();
		});
		if (this.changeFoot) {
		...
		this.footBackKey = BackHandler.addEventListener("back", () => {
			...
			if (routeName.indexOf('Index') != -1 || routeName.indexOf('Login') != -1 || routeName.indexOf('Home') != -1) {
				//console.log('禁用back页面');
				return true;
			} else {
				//console.log('启用back页面');
				return false;
			}
		});
					...
					if (tempParams.user.code.toString() == '200' && tempParams.user.original.toString() == '0') {
						...
						if (tempParams.user.add.toString() == '1') {
							tempParams.user.addUserInfo.map((item, index) => {
								...
								Sqlite.saveUser(this.state.basic.userId, userOjb, () => {
									i++;
									// console.log('增加人员' + item.trueName + "成功");
									this.checkCall(i, length, tempParams.deptId, tempParams.version);
								})
							});
						}
						if (tempParams.user.update.toString() == '1') {
							tempParams.user.updateUserInfo.map((item, index) => {
								...
								Sqlite.saveUser(this.state.basic.userId, userOjb, () => {
									i++;
									// console.log('修改人员' + item.trueName + "成功");
									this.checkCall(i, length, tempParams.deptId, tempParams.version);
								})
							});
						}
						...
						tempParams.user.originalUserInfo.map((item, index) => {
							...
							Sqlite.saveUser(this.state.basic.userId, userOjb, () => {
								// console.log('保存人员：' + item.trueName + " 成功");
								i++;
								this.checkCall(i, length, tempParams.deptId, tempParams.version);
							});
						});
				...
				if (responseJson.code.toString() == '200') {
					...
					if (tempParams.tree) {
						...
						Sqlite.saveValue(this.state.basic.userId, 'tree_' + tempParams.deptId, JSON.stringify(tempParams.tree), () => {
							...
							setTimeout(() => {
								DeviceEventEmitter.emit('updateAddressThird');//刷新通讯录页面
							}, 50);
						});
					}
					if (tempParams.dept.code.toString() == '200' && tempParams.dept.original.toString() == '0') {
		...
		this.invNotice = DeviceEventEmitter.addListener('invNotice',(param)=>{
			this._getLocalTalker_inform(param.tempMsgInfo,param.num,true);
		})
	...
	//播放/暂停语音播放
	playVoiceMessage = () => {
		// console.log('enter play sound');
		const s = new Sound('shake.wav', Sound.MAIN_BUNDLE, (e) => {
			// console.log('sound call back');
			if (e) {
				console.log('播放失败');
				console.log(e);
				return;
			...
			Sqlite.saveValue(this.state.basic.userId, 'userList_' + deptId, version, () => {
				setTimeout(() => {
					if (deptId == '0') {
						...
						DeviceEventEmitter.emit('updateAddressSec');//刷新通讯录页面
					}
				}, 50);
			});
		}
	...
	componentWillUnmount() {
		...
		if (Platform.OS == 'android') {
			// this._FootXmppErrorListener.remove();
			// this._FootXmppListener.remove();
			// this._FootMsgListener.remove();
			// this._FootPreListener.remove();
			// this.appStateLis.remove();//移除前后台监听
		} else {
			//ios监听卸载
			this.XMPPReconnnect.remove();
			this.messageSendNotification.remove();
	...
	/** 监听到主动加入结点消息后，如果存在该条记录，更新数据库，如不存在则创建*/
	iOS_refreshCurrentMessageSql = (tempMsgInfo) => {
		//入库通知类型
		Sqlite.selectTalkers(this.state.basic.userId, Global.loginUserInfo.jidNode, (data) => {
			if (data.length > 0) {
				Sqlite.saveMessage(this.state.basic.userId, Global.loginUserInfo.jidNode, 'text', '您有新的群申请待处理', '',
					...
					(data) => {
						...
						//更新未读条数
						Sqlite.updateTalker(this.state.basic.userId, Global.loginUserInfo.jidNode, 1, false, null,false, (data) => {
							//console.log(data);
							DeviceEventEmitter.emit('refreshPage', 'refresh');
						});
						Sqlite.updateTalkerIsAnted(this.state.basic.userId, Global.loginUserInfo.jidNode, false, () => {
							DeviceEventEmitter.emit('refreshPage', 'refresh');
						});
					}
				);
			} else {
				Sqlite.saveTalker(this.state.basic.userId, 3, '', Global.loginUserInfo.jidNode, '通知', '',
					false,
					false,
					false,
					() => {
						Sqlite.saveMessage(this.state.basic.userId, Global.loginUserInfo.jidNode, 'text', '您有新的群申请待处理', '',
							Global.loginUserInfo.jidNode + new Date().getTime(), null,
							(data) => {
								DeviceEventEmitter.emit('refreshPage', 'refresh');
								//更新角标，播放声音
								this.playVoiceMessage();//播放声音
	...
	receiveiOSNodeMessage = (tempMsgInfo) => {
		...
		if (tempMsgInfo.mode == 'join' && tempMsgInfo.applicant != Global.loginUserInfo.jidNode) {
			this.iOS_refreshCurrentMessageSql(tempMsgInfo);
		}
		...
		if (tempMsgInfo && tempMsgInfo.mode == 'SETUNMUTE' && tempMsgInfo.effect == this.state.basic.jidNode) {
			...
			Sqlite.selectTalkers(this.state.basic.userId, body.basic.toId, (data) => {
				...
				if (data.length > 0) {
					Sqlite.saveMessage(this.state.basic.userId, body.basic.toId, tempType, msgContent, '', body.id, null, (data) => {
						...
						//console.log(data);
						//this.fetchMessage();
						//console.log('单聊第一次发送人jid:'+body.basic.fromId);
						//console.log('单聊第一次登录人jid:'+this.state.basic.jidNode);
	...
	currentGroupMessageIsExist = (body, isanted, tempType, tempTo, msgContent, tempEffectId) => {
		if ((body.occupant && ((body.occupant.state == 'OWNLEAVE' || body.occupant.state == 'SETLEAVE') && tempEffectId == this.state.basic.jidNode)) || (body.occupant && body.occupant.state == 'DISABLEGROUP')) {
			...
			if (body.occupant.state == 'DISABLEGROUP' || body.occupant.state == 'SETLEAVE') {
							...
							() => {
								Sqlite.saveMessage(
									...
									(data) => {
										//console.log(data);
										//this.fetchMessage();
										//console.log('群聊发送人jid:'+body.basic.userId);
										//console.log('群聊登录人jid:'+this.state.basic.jidNode);
										DeviceEventEmitter.emit('refreshPage', 'refresh');
										//如果监听到的消息不是我自己发的并且不是当前所在群，更新角标，播放声音
										if (body.basic.userId != this.state.basic.jidNode && groupId != body.basic.groupId) {
											if ((body.occupant.state == 'INVITEJOINROOM' || body.occupant.state == 'OPENGROUP' || body.occupant.state == 'INVITEJOINAUDITROON') && body.basic.formId != this.state.basic.userId) {
	...
	_groupGeneralMessage = (message, messageBody) => {
		...
		// }
			...
			console.log("文本、语音、图片、文件、除自己外的其它成员被移除群组");
	...
	_groupSubscriptionMessage = (message) => {
		...
		if (tempMsgInfo && tempMsgInfo.mode == 'SETLEAVE') {
		...
		else if (tempMsgInfo && tempMsgInfo.mode == 'INVITEJOINROOM') {
			//被邀请加入非审核群消息
			//为了传参发的
			let url = Path.getGroupDetailNew + '?roomJid=' + tempMsgInfo.roomjid + '&uuId=' + this.state.uuid + '&ticket=' + this.state.ticket + '&currentJidNode=' + this.state.basic.jidNode + '&userId=' + this.state.basic.userId;
			FetchUtil.netUtil(url, {}, 'GET', this.props.navigation, '', (responseJson) => {
				if (responseJson.code.toString() == '200') {
					...
					if (tmpDetail.banRoom == '0') {
						//收到邀请后主动出席
						XMPP.joinRoom(tempMsgInfo.roomjid + Path.xmppGroupDomain, this.state.basic.jidNode);
					}
				}
			});
		}
			...
			//审核入群通知消息
			this._getLocalTalker_inform(tempMsgInfo,1,false);
		}
		...
		else if(!message.body && message.extenEl.indexOf('<changeinfo xmlns=\'jabber:client\'></changeinfo>') != -1){
			/** 更新群头像和名称 **/
			let url = Path.getGroupDetailNew + '?roomJid=' + message.from.split('@')[0] + '&uuId=' + this.state.uuid + '&ticket=' + this.state.ticket + '&currentJidNode=' + this.state.basic.jidNode + '&userId=' + this.state.basic.userId;
			FetchUtil.netUtil(url, {}, 'GET', this.props.navigation, '', (responseJson) => {
				if (responseJson.code.toString() == '200') {
	...
	_saveLocalMessage_inform = (tempMsgInfo,num,coverFlag) => {
		Sqlite.saveMessage(this.state.basic.userId, Global.loginUserInfo.jidNode, 'text', '您有新的群申请待处理', '',
			...
			(data) => {
				...
				this.playVoiceMessage();//播放声音
				this.setState({
					newMeesageTab: this.state.newMeesageTab + num
				});
				//更新未读条数
				Sqlite.updateTalker(this.state.basic.userId, Global.loginUserInfo.jidNode, num, false, null,coverFlag, (data) => {
					//刷新消息列表
					this._refreshMessageList(0, 0);
				}, (err) => {
					//刷新消息列表
	...
	/**
	 * 获取本地数据库群通知Talker，本地存在时直接操作message，本地不存在时先生成新的talker
	 * @param tempMsgInfo message完整消息
	 * @private
	 */
	_getLocalTalker_inform = (tempMsgInfo,num,coverFlag) => {
		//入库通知类型
		Sqlite.selectTalkers(this.state.basic.userId, Global.loginUserInfo.jidNode, (data) => {
			if (data.length > 0) {
				this._saveLocalMessage_inform(tempMsgInfo,num,coverFlag);
			} else {
				Sqlite.saveTalker(this.state.basic.userId, 3, '', Global.loginUserInfo.jidNode, '通知', '',
					false,
					false,
	...
	//判断设备锁启用start
	_getCurrentState = () => {
		// console.log('foot getcurrentState:' + AppState.currentState);
		...
		if (Platform.OS == 'ios') {
				...
				if (this.state.nowPage.indexOf('HandLock') != -1) {
					...
					cookie.get('isUpload').then((str) => {
						...
						if (!str) {//前台时判断是否存入cookie“isUpload”，未存入判断是否弹出设备锁
							// console.log("_appStatus:" + _appStatus);
			...
			if (AppState.currentState == 'active') {//当前为前台
				...
				if(!XMPP.isConnected && this.isReconnectXMPP){
                    NativeModules.JudgeNetwork.judgeNetwork(Path.network_ip,(flag)=>{
        ...
        NativeModules.JudgeNetwork.judgeNetwork(Path.network_ip,(flag)=>{
            if(flag == "true"){
                ...
                if(pingNum < 10){
                    ...
                    this.isReconnectXMPP = true;
                    DeviceEventEmitter.emit('headTitle', "消息(无网络)");
                    if(this.tmpTimeout){clearTimeout(this.tmpTimeout)};
				}else{
                    DeviceEventEmitter.emit('headTitle', "消息(无网络)");
                    if(this.tmpTimeout){clearTimeout(this.tmpTimeout)};
                    this.tmpTimeout = setTimeout(this.netWorkPing, 5000);
				}
            }
        });
	}
					...
					<TabNavigator.Item
						...
						renderBadge={() => {
							if (this.state.newMeesageTab == 0) {
								...
								// console.log(this.state.newMeesageTab)
