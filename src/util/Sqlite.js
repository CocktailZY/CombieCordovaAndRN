...
export default Sqlite = {
	...
	saveUser(userId, userOjb, callback) {
		...
		//console.log(writeObj(userOjb));
		//console.log(userOjb.jid_node);
	...
	saveDepartment(userId, deptId, deptName, previousId, callback) {
		...
		db.transaction((tx) => {
			tx.executeSql(sql, str, (tx, results) => {
					//console.log("部门入库：" + deptName + " 成功");
				}, (err) => {
					//console.log("部门入库：" + deptName + " 失败" + writeObj(err));
	...
	saveTalker(userId, type, jid, jid_node, trueName, imageName, isPromote, isnoremind, isanted, callback) {
		...
		//console.log('saveTalker接到的jidNode===' + jid_node);
		//console.log('saveTalker接到的trueName===' + trueName);
	...
	updateTalkerUnread(userId,jid_node,num,callback){
        ...
        this.selectTalkers(userId, jid_node, (talker) => {
            ...
            db.transaction((tx) => {
                tx.executeSql(sql, str, (tx, results) => {
                        //console.log('updateTalkerUnread:' + n);
                    }, (err) => {
                        ConsoleUtil.console(err);
                    }
                );
	...
	updateTalkerPromote(userId, type, jid, jid_node, trueName, imageName, isPromote, is_no_remind, is_anted, callback) {
		...
		this.selectTalkers(userId, jid_node, (talker) => {
			...
			if (isPromote == true) {
				...
				// console.log("Sqlite.js::::::::设置置顶：" + time);
				// ConsoleUtil.console("Sqlite.js::::::::设置置顶：" + time);
			} else {
                // console.log("Sqlite.js::::::::取消置顶：" + time);
			...
			if (talker && talker.length > 0) {
				db.transaction((tx) => {
					...
					// console.log("Sqlite.js::::::::置顶优先级更新成功");
		...
		db.transaction((tx) => {
			tx.executeSql(sql, str, (tx, results) => {
                ...
                // console.log(e-d);
					...
					//console.log("selectTalker查询message列表结果：");
					//console.log(talkers);
			...
			console.log("qlite.js::::::::查询message列表错误，错误详情：" + writeObj(err));
		}, () => {
	...
	selectUnreadMsgTatol(userId, callback) {
		...
		db.transaction((tx) => {
			tx.executeSql(sql, str, (tx, results) => {
					let num = 0;
					let len = results.rows.length;
					// console.log('talker数量：'+len);
					for (let i = 0; i < len; i++) {
						// console.log(results.rows.item(i).unread_number);
						num = results.rows.item(i).unread_number ? results.rows.item(i).unread_number : 0;
					}
					// console.log("qlite.js::::::::未读总条数：" + num);
					callback(num);
				}, (err) => {
				console.log(err);
				}
			...
			console.log("qlite.js::::::::查询未读条数错误，错误详情：" + writeObj(err));
		}, () => {
	...
	deleteTalker(userId, jid_node, callback) {
		...
		db.transaction((tx) => {
			...
			console.log("qlite.js::::::::删除talker错误，错误详情：" + writeObj(err));
		}, () => {
			// console.log("qlite.js::::::::删除talker成功");
			callback();
		})
	},
	/**
	...
	saveMessage(userId, jid, type, content, sendName, msgId, sendTime, callback) {
		// console.log("进入saveMessage");
		//console.log('saveMessage接到的jidNode===' + jid);
		if (userid == "") {
			this.initConstant(userId);
		}
		if (!db) {
		...
		this.selectMessage(userId, jid, (data) => {
	 ...
	 * @param callback
	 */
	selectMessage(userId, jid, callback) {
		if (userid == "") {
			this.initConstant(userId);
		}
		if (!db) {
	...
	saveOfflineTime(userId, roomJid, lastTime) {
		...
		// console.log('saveTime接到的jidNode===' + roomJid);
		// console.log('saveTime接到的lastTime===' + lastTime);
		if(!lastTime){
			...
			if (data && data.length > 0) {
				...
				db.transaction((tx) => {
					tx.executeSql(sql, [lastTime, roomJid], (tx, results) => {
							//console.log("update time updateSuccess");
						}, (err) => {
							ConsoleUtil.console("qlite.js::::::::" + err);
						}
					);
