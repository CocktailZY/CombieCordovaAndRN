...
import {
	StyleSheet, Text, View, TextInput, Platform, TouchableOpacity, ScrollView
} from 'react-native';
	...
	getParticipantsList(body, number, type) {
		...
		FetchUtil.netUtil(Path.getParticipantsList + ParamsDealUtil.toGetParams(params), {}, 'GET', this.props.navigation, '', (data) => {
			if (data.code.toString() == '200') {
				...
				let selectNotArr = this.state.selectNotArr;
				let users = [];
				box.users.map((item) => {
				...
				// for (let i in this.state.selectedArrBox) {
				// 	for (let j in selectedArr) {
				// 		if (this.state.selectedArrBox[i].jidNode == selectedArr[j].jidNode) {
				// 			// this.state.selectNotArr.splice(j, 1);
				// 			this.state.selectedArrBox.splice(i, 1);
				// 		}
				// 	}
				//
				// 	for (let k in this.state.selectedParticipants) {
				// 		if (this.state.selectedArrBox[i].jidNode == this.state.selectedParticipants[k].jidNode) {
				// 			this.state.selectedArrBox.splice(i, 1);
				// 		}
				// 	}
				// }
		...
		FetchUtil.netUtil(Path.getUserList + ParamsDealUtil.toGetParams(params), {}, 'GET', this.props.navigation, '', (data) => {
			if (data.code == '200') {
				...
				if (users.length > 0) {
					...
					this.refs.toast.show('未查询到搜索内容', DURATION.LENGTH_SHORT);
				}
			}
		});
	}
	...
	render() {
		...
		return (
			<View style={styles.container}>
				...
				{
					this.state.selectedArrBox.length > 0 ?
