...
import {
	...
	Platform, TouchableOpacity, Dimensions, ScrollView, DeviceEventEmitter,
...
export default class Job extends Component {
	...
	componentDidMount(){
		if(this.jobBackKey){this.jobBackKey.remove();};
				...
				<ImageBackground style={{flex: 1, padding: 10}} source={require('../../images/job_bg.jpg')}
					...
					<ScrollView
						...
						<TouchableWithoutFeedback style={{flex: 1, marginTop: 10}}
																			onPress={()=>{HandlerOnceTap(() => this._jobNavigite('Invite'))}}>
						...
						{
							Path.elecTable ? (
								<TouchableWithoutFeedback style={{flex: 1}} onPress={()=>{HandlerOnceTap(() => this._jobNavigite('Invite'))}}>
									<View style={[styles.jobBtn, {
										flex: 1,
										justifyContent: 'center',
										alignItems: 'center',
