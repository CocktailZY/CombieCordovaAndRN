...
export default class FriendDetail extends Component {
		...
		FetchUtil.netUtil(Path.getFriendDetail + ParamsDealUtil.toGetParams(params), {}, 'GET', this.props.navigation, '', (res) => {
			...
			if (res.code == '200') {
				...
				this.setState({
					...
					// console.log(this.state.friendJidNode, this.state.friendUserId, this.state.isFriendFlag);
	...
	iqCallBack = (message, addMessage) => {
		...
		if (message.type == 'result') {
			...
			//跳转好友列表-----------------
			//this.props.navigation.navigate('Friend',{ticket:this.state.ticket,uuid:this.state.uuid,basic:this.state.basic});
			this._fetchFriendDetail();
	...
	setContactsCallBack = (data) => {
		if (data.code.toString() == '200') {
			...
			// let sendIQ = XmlUtil.sendRemoveIQ(this.state.friendDetail.jid);
			// XMPP.sendStanza(sendIQ);

			// let sendPresence = XmlUtil.sendPresence(this.state.basic.jid, this.state.friendDetail.jid);
			// XMPP.sendStanza(sendPresence);
			// Sqlite.deleteTalker(this.state.basic.userId,this.state.friendDetail.jidNode,()=>{
			//     this.refs.toast.show('删除成功', DURATION.LENGTH_SHORT);
			//     DeviceEventEmitter.emit('refreshFriendList');
			//     DeviceEventEmitter.emit('refreshPage');
			// });
			//} else {
			//ios处理
			// XMPP.XMPPRemoveFriend(
			//     {
			//         'friendJid': this.state.friendDetail.jid,
			//         'jid': this.state.basic.jid,
			//         'uuid': this.state.uuid
			...
			//}
			this.setState({
				// friendUserId: this.state.friendDetail.userId,
	...
	async requestCallPhonePermission(mark) {
		try {
			...
			if (granted === PermissionsAndroid.RESULTS.GRANTED) {
				...
				// console.log("You can not call phone")
			}
		} catch (err) {
			// console.warn(err)
		}
		...
		return (
			<View style={styles.container}>
				...
				<Header
					...
					onPressRightBtn={this.state.basic.jidNode != this.state.friendDetail.jidNode ? (this.state.commonStatus == '1' ? this._removeContacts : this._setContacts) : null}
					backTitle={'返回'}
					title={'详细资料'}
				/>
				<ScrollView
						...
						<View style={[styles.detailsPictrue, styles.detailsImage]}>
							<Image
								source={{
									...
									// Path.headImg + '?fileName=' + this.state.friendDetail.photoId + '&userId=' + this.state.basic.userId + '&uuId=' + this.state.uuid + '&ticket=' + this.state.ticket
								}}
