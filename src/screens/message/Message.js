			...
			<TouchableHighlight
				...
				<View style={styles.itemBody}>
					<View style={styles.itemLeft}>
						{myData.ttype == 3 ? (
							<View style={[{width: 46, height: 46, borderRadius: 4, backgroundColor:'#ffbe76'},baseStyles.row_col_center]}>
        ...
        FetchUtil.netUtil(Path.receivedJobs, params, 'POST', this.props.navigation, {
            ...
            userId: this.state.basic.userId
						...
						if(parseInt(responseJson.data.noticeNum) > 0){
							//通知
							DeviceEventEmitter.emit('invNotice',{tempMsgInfo:{applicant:'notice'},num:parseInt(responseJson.data.noticeNum)});
						}
                ...
                if (item.ttype == 1) {
                    ...
                    FetchUtil.netUtil(url, {}, 'GET', this.props.navigation, '', (responseJson,tip) => {
                        if (responseJson == 'tip') {
                            ...
                            mute = responseJson.data[0].isMute;
                            let obj = {
                                backPage: 'Group',
                                basic: this.state.basic,
                                token: this.state.token,
