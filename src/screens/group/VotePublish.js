...
import {
	...
	NativeModules, DeviceEventEmitter, Modal, Keyboard,
} from 'react-native';
...
export default class VotePublish extends Component {
	...
	_PostVote = () => {
		...
		if (body.title == '') {
			...
			this._toast('选项内容不能为空或含有非法字符!');
		} else if (temp.length > 20) {
			this._toast('请输入有效投票选项,选项内容不得超过20个字符!');
		} else if (tempcategory) {
			this._toast('选项不得包含非法字符!');
		} else if (!flag) {
			this._toast('投票选项至少两项,选项相同为一项！');
		...
		// 		}else{
		// 			this.refs.toast.show('您没有权限发布投票', DURATION.LENGTH_SHORT);
		//
		// 		}
		// 	}
		// });
	...
	_selectVoteDay = (date) => {
		...
		this.setState({
			...
			// console.log(this.state.voteBody)
		});
	};
