...
export default class ActivityPublish extends Component {
	...
	_PostActivity = () => {
		...
		if (body['title'] == '') {
			...
			this._toast('地点名称不得包含非法字符');
		}else if(body['address'].length > 50){
			this._toast('地点名称不得超过50个字');
		}else if (body['cost'] < 0) {
			this._toast('活动费用不得小于0');
		}
		// else if(body['cost'].length > 5){
		// 	this._toast('活动费用不得超过5位数');
		// }
				...
				FetchUtil.netUtil(url, body, 'POST', this.props.navigation, {
					...
					if (responseJson.code.toString() == '200') {
						...
						if (Platform.OS === 'ios') {
							...
							// console.log('发送活动message报文：' + sendActivity);
		...
		// 		}else{
		// 			this.refs.toast.show('您没有权限发布活动', DURATION.LENGTH_SHORT);
		//
		// 		}
		// 	}
		// });
	...
	_inputActivity = (text, type) => {
		...
		if(type == 'cost'){
			...
			// console.log(ToolUtil.moneyLimit(tempBody,5));
	...
	_selectActivityDay = (date) => {
		...
		this.setState({
			...
			// console.log(this.state.activityBody)
		});
	};
				...
				if (trueType.indexOf(imageType) > -1) {
					...
					fetch(url, {
						...
						if (responseData.code.toString() == '200') {
							...
							this.setState({topicBody: body, posterUrl: bese64Image,activityBody:body});
						} else {
							Alert.alert(
								'提醒',
								'系统异常,请退出重试',
