...
export default class AtMember extends Component {
	...
	_searchFriend = () => {
		...
		let url = Path.getGroupMember + '?occupantNick=' + this.state.searchText + '&uuId=' + this.state.uuid + '&ticket=' + this.state.ticket + '&roomJid=' + this.state.room.roomJid + '&currentJidNode=' + this.state.basic.jidNode + '&userId=' + this.state.basic.userId;
	...
	fetchData = () => {
		let url = Path.getGroupMember + '?uuId=' + this.state.uuid + '&ticket=' + this.state.ticket + '&roomJid=' + this.state.room.roomJid + '&currentJidNode=' + this.state.basic.jidNode + '&userId=' + this.state.basic.userId;
		FetchUtil.netUtil(url, {}, 'GET', this.props.navigation, '', (data) => {
			if (data.code == '200') {
				this.setState({
