...
export default class Chat extends Component {
	...
	componentDidMount() {
			...
			(data) => {
				if (data.status == "OK") {
					RNFS.exists(audioPath).then((value) => {
						if (value == true && endTime - beginTime >= 1000) {
		...
		if (this.props.navigation.state.params.backPage == 'Group') {
			Sqlite.saveOfflineTime(this.state.basic.userId, this.state.room.roomJid, new Date().getTime());
		}
		...
		if (Platform.OS == 'ios') {
			...
			this.groupMessageNotification = DeviceEventEmitter.addListener('groupMessageToRN', (messageBody) => {
				...
				} else if (messageBody.msgInfo && messageBody.msgInfo.mode == 'SETUNMUTE' && messageBody.msgInfo.effect == this.state.basic.jidNode && messageBody.msgInfo.roomjid == this.state.room.roomJid) {
				...
				if (messageBody.isRemoveMsg == 'true') {
					...
					if (messageBody.body) {
						...
						if (groupMesbody.occupant && groupMesbody.occupant.state == 'DISABLEGROUP') {
	...
	_loadMessage = (message) => {
		...
		if (messageBody && messageBody.basic && messageBody.basic.type && messageBody.basic.type != 'privateChat' && messageBody.basic.groupId == this.state.room.roomJid) {
			...
			/** 私聊普通消息处理（不包括撤回消息） **/
			console.log("私聊普通消息处理（不包括撤回消息）");
			this._privateGeneralMessage(message, messageBody);
		} else if (!messageBody && message.removeMsgId && message.from.indexOf('muc') == -1) {
			/** 私聊撤回消息处理 **/
			console.log("私聊撤回消息处理");
		...
		if (false) {
			...
			if (message.msgInfo) {
				let tempMsgInfo = JSON.parse(message.msgInfo);
				if (tempMsgInfo.mode == 'SETLEAVE') {
					tempEffectId = tempMsgInfo.effect;
					if (tempEffectId == this.state.basic.jidNode && this.state.room.roomJid == tempMsgInfo.roomjid) {
						Alert.alert(
							'提醒',
							'您被移出该群组',
							[
				...
				//接到禁言消息
				if (tempMsgInfo.mode == 'SETMUTE' && tempMsgInfo.roomjid == this.state.room.roomJid) {
					tempEffectId = tempMsgInfo.effect;
					if (tempEffectId == this.state.basic.jidNode) {
				...
				//接到取消禁言消息
				if (tempMsgInfo.mode == 'SETUNMUTE' && tempMsgInfo.roomjid == this.state.room.roomJid) {
					tempEffectId = tempMsgInfo.effect;
					if (tempEffectId == this.state.basic.jidNode) {
						let tempRoom = this.state.room;
						tempRoom.mute = '0';//取消禁言
			...
			let tempBody = typeof message.body == 'string' ? message.body : JSON.stringify(message.body);
			if (typeof message != 'string' && message.body) {
				if ((tempBody.indexOf('{') != -1) && (tempBody.indexOf('removemsg') == -1)) {
					let tempItemObj = {};
					let tempBodyJson = JSON.parse(tempBody);
					let formIdTemp = JSON.parse(tempBody).basic.fromId;
					if (this.state.backPage == 'Message' && formIdTemp) {
						if ((formIdTemp == this.state.basic.jidNode && JSON.parse(tempBody).basic.toId == this.state.friendDetail.jidNode) || formIdTemp == this.state.friendDetail.jidNode) {
							if (tempBodyJson.type && tempBodyJson.type == '2') {
								if (tempBodyJson.content.file[0].listFileInfo[0].showPic == 'audio') {
									tempItemObj['isPlay'] = new Animated.Value(0);
								}
							}
							tempItemObj['body'] = tempBody;
							if (formIdTemp == this.state.basic.jidNode) {
								tempItemObj['sendType'] = 'to';
							} else {
							...
							let body = JSON.parse(tempBody);
							if (body.basic.fromId == this.state.friendDetail.jidNode) {
								let messageType = '';
								// let msgContent = '';
								...
								this._saveToDB(1, this.state.friendDetail.jidNode, this.state.friendDetail.trueName, this.state.friendDetail.photoId, '', '', '', messageType, '', message.id);//入库当前朋友信息
							}
						}
						// }
					} else {
						//群聊
						let body = (typeof message.body == 'object') ? message.body : JSON.parse(message.body);
						if (body.basic.userId != this.state.basic.jidNode || !this.state.isNotPC) {//不是我自己发的或者是PC发的消息拼接
							let tempArr = [];
							if (body.basic.groupId == this.state.room.roomJid) {
								let tempRoom = JSON.parse(JSON.stringify(this.state.room));
								// if (body.occupant && body.occupant.state == 'SETMUTE' && body.occupant.effect == this.state.basic.jidNode) {// &&
								// 	tempRoom.mute = '1';//被禁言
								// } else {
								// 	tempRoom.mute = '0';
								// }
								if (body.occupant) {
									if (body.occupant.state == 'SETMEMBER' || body.occupant.state == 'SETLEAVE' || body.occupant.state == 'SETADMIN') {
										DeviceEventEmitter.emit('refreshGroupDetail');
									}
									if (body.occupant.state == 'DISABLEGROUP' && body.basic.groupId == this.state.room.roomJid && body.basic.userId != this.state.basic.jidNode) {
					...
					if (message.src && message.src.indexOf('removemsg') != -1) {
						this.setState({
							currentNum: 0
						}, () => {
							nowPageFlag = 0;
	...
	_groupGeneralMessage_General = (message, messageBody) => {
		console.log(this.state.isNotPC);
		//文本消息||其他人发的消息||PC发的消息
	...
	_groupSubscriptionMessage = (message) => {
		...
		// if(tempMsgInfo && tempMsgInfo.mode == 'DISABLEGROUP'){
		//     console.log("禁用群组");
		// }
		...
		if (tempMsgInfo && tempMsgInfo.mode == 'SETLEAVE' && tempMsgInfo.roomjid == this.state.room.roomJid && tempMsgInfo.effect == this.state.basic.jidNode) {
			console.log("被移除群组");
			// if (tempMsgInfo.effect == this.state.basic.jidNode && this.state.room.roomJid == tempMsgInfo.roomjid) {
			Alert.alert(
				'提醒',
				'您被移出该群组',
				[
	...
	_onRefresh = () => {
		...
		if (this.state.backPage == 'Message') {
			this.setState({
				...
				// currentNum: this.state.currentNum + pageSize > this.state.pageTotal ? this.state.currentNum : this.state.currentNum + pageSize
			}, () => {
				if (Platform.OS == 'ios') {
		...
		if (this.state.room.mute == '1') {
			...
			if (Platform.Version < 23 || Platform.OS == 'ios') {
				...
				PermissionUtil.requestAndroidPermission(
					PermissionsUtil.Permissions.camera, (value) => {
						if (typeof value == "boolean" && value) {
							...
							for (let key in value) {
								if (key == PermissionsUtil.Permissions.camera && value[key]) {
									this.openImagePicker();
								}
							}
						} else {
		...
		ImagePickerManager.launchImageLibrary(photoOptions, (response) => {
			if (response.didCancel) {
				...
				if (trueType.indexOf(imageType) > -1) {
					...
					let newMsgId = UUIDUtil.getUUID().replace(/\-/g, '') + 'GroupMsg';
					let newMsgIdSigle = UUIDUtil.getUUID().replace(/\-/g, '');
					let files = [{status: 'success', listFileInfo: [{showPic: 'img'}]}, {imageUrl: response.uri}];//本地图片路径
					...
					this.setState({
						...
						let file = {
							...
							name: response.fileName ? encodeURIComponent(response.fileName) : 'image.' + imageType
						};
						formData.append("file", file);
						let stype = this.state.backPage == 'Group' ? 'groupchat' : 'sglc';
						let to = this.state.backPage == 'Group' ? this.state.room.roomJid : this.state.friendDetail.jidNode
	...
	takePicture = () => {
		...
		ImagePickerManager.launchCamera(photoOptions, (response) => {
			...
			if (response.didCancel) {
				...
				if (response.error) {
					this.refs.toast.show('请检查您的相机权限是否开启', 3000);
					return;
				}
				let newMsgId = UUIDUtil.getUUID().replace(/\-/g, '') + 'GroupMsg';
				let newMsgIdSigle = UUIDUtil.getUUID().replace(/\-/g, '');
						...
						fetch(url, {
							...
							.then((responseData) => {
								if (responseData.code.toString() == '200') {
									...
									let fileType = fileTypeReturn.getfileType(fileName);
									let tip = '';
									if (fileType == 'img') {
										tip = '[图片]';
									...
									if (this.state.backPage == 'Group') {
										PushUtil.pushGroupNotification(this.state.basic, this.state.ticket, this.state.room.roomJid, this.state.uuid, this.state.basic.trueName + ':' + tip, this.state.room.roomName, this.props.navigation);
									} else {
										PushUtil.pushSingleNotification(this.state.basic, this.state.ticket, this.state.friendDetail.jidNode, this.state.uuid, this.state.friendDetail.userId, tip, this.state.basic.trueName, this.props.navigation);
									}
								} else if (responseData.code.toString() == '-32') {
						...
						Sqlite.saveMessage(
							...
							(data) => {
								this.setState({
									...
									//更新离线时间
									// Sqlite.saveOfflineTime(this.state.basic.userId, roomJid, new Date().getTime());
									//DeviceEventEmitter.emit('refreshPage', 'refresh');
								})
							}
						);
...
//文本消息
	myselfText = (type, content, oid, item, msgId) => {
		...
		let emojiView = null;
		if (isEmoji) {
			emojiView = <Text>{textArr}</Text>
		} else {
			emojiView =
				<Text style={{color: type == 'self' ? '#fff' : '#000', lineHeight: 20}} selectable={false}>{trueText}</Text>
		}
...
//语音消息
	myselfAudio = (type, fileId, soundTime, oid, content, item) => {
		// delete item.isPlay;
		if (item.isPlay) {
			// console.log(typeof item.isPlay);
		...
		// let isPlay = item.isPlay ? item.isPlay : new Animated.Value(0);
		// console.log(isPlay);
		return (
			<View style={{justifyContent: 'center'}}>
				<View style={[{position: 'relative'}, type == 'self' ? {paddingRight: 3} : {paddingLeft: 3}]}>
								...
								>
									{
										type == 'other' ? (
											<View style={{flexDirection: 'row'}}>
												...
												<Animated.View style={{
													backgroundColor: '#FFFFFF', zIndex: 999, height: 15, right: 12, opacity: item.isPlay
									...
									}}>{soundTime}"</Text>
									{
										type == 'self' ? (
											<View style={{flexDirection: 'row'}}>
												<Animated.View style={{
													backgroundColor: '#549dff', zIndex: 999, height: 15, left: 12, opacity: item.isPlay
												}}><Text style={{color: 'transparent'}}>{'1'}</Text></Animated.View>
							...
							>
								...
								<View style={{flex: 1, justifyContent: 'center'}}>
									<Text numberOfLines={1}
												style={{fontSize: 15, color: '#333', textAlign: 'left', marginBottom: 3}}>{file.fileName}</Text>
									<Text style={{
										fontSize: 10,
										color: '#777',
										textAlign: 'left'
	...
	myselfView(type, body, index, item) {
		...
		tempViewBox = (
			<View key={index} style={[styles.sendTypeView, {
				...
				// alignItems:'center',
				// paddingLeft: type == 'self' ? 130 : 0,
				// paddingRight: type == 'other' ? 130 : 0
			}]}>
				{
					type == 'other' ?
						<TouchableWithoutFeedback style={{width: 37, justifyContent: 'flex-start'}} onPress={() => {
							...
							<Image
								cache='reload'
								source={{
									uri: Path.headImgNew + '?uuId=' + this.state.uuid + '&ticket=' + this.state.ticket + '&userId=' + this.state.basic.userId + tempUserId //'&imageName=' + tempUserId + '&imageId=' + tempUserId+ '&sourceType=singleImage&jidNode=' + (this.state.backPage == 'Message' ? this.state.friendDetail.jidNode : body.basic.userId)
									// uri: Path.baseUrl+ '/singleChat/queryPhotoAddress' + '?uuId=' + this.state.uuid + '&ticket=' + this.state.ticket + '&userId=' + this.state.basic.userId + tempUserId
								}}//(this.state.backPage == 'Message' ? '&fileName=' + body.basic.photoId : '&jidNode=' + body.basic.userId)  tempHead[body.basic.userId] ? tempHead[body.basic.userId] :
				...
				<View
					// 		()=>{
					// 			this.setState({
					// 				isTextShow: true,//type == 'self' ? true : false,
					// 				isTextShowID: body.id//type == 'self' ? oid : '',
					// 			}, () => {
					// 				console.log(this.state.isTextShow);
					// 				console.log(this.state.isTextShowID);
					// 				console.log(this.state.isTextShow && this.state.isTextShowID === body.id);
					// 			})
					// }}
					style={[
						(type == 'other' ? {alignItems: 'flex-start', paddingRight: 130} : {
							alignItems: 'flex-end',
							paddingLeft: 130
						}),
						{flex: 1}
...
//render处理 end
						...
						<Pdf
							...
							onError={(error) => {
								this.setState({pdfInsideModalVisible: false}, () => {
									this.refs.toast.show('文件预览失败', DURATION.LENGTH_SHORT);
								});
							}}
							enablePaging={false}
							onPageSingleTap={() => {
					...
					{/*<View style={{flex:1}}>*/}
						<FlatList
							...
							ListHeaderComponent={() => {
								...
								// if (this.state.isRefresh && this.state.data.length > 0) {
								// 	return (<View style={styles.footer}>
								// 		<ActivityIndicator/>
								// 		<Text style={styles.footerText}>
								// 			{'正在加载更多...'}
								...
								// 	</View>)
								// } else {
								// 	if ((this.state.data.length > 0) && (this.state.nowPageNum > this.state.totalPage)) {
								// 		return (<View style={styles.footer}>
								// 			<Text style={styles.footerText}>
								// 				{'没有更多了'}
								// 			</Text>
							...
							onContentSizeChange={(contentWidth, contentHeight) => {
								if (this.state.isNewMsg || this.state.nowPageNum == 1) {
									...
									// }
									// if(this.state.backPage == 'Message'){
									// 	this._scrollView.scrollToIndex({animated:true,index:(this.pageNumFlag ? Path.pageSize+this.timeNum-1 : this.flagPageNum-1),viewPosition: 0});
									// }else{
									// 	this._scrollView.scrollToIndex({animated:true,index:(this.pageNumFlag ? Path.pageSizeNew+this.timeNum-1 : this.flagPageNum-1),viewPosition: 0});
									// }
									...
									// 	x: 0,
									// 	y: contentHeight - scrollViewHeight - (height / size),
									// 	animated: false
									// })
								}
							...
							onContentSizeChange={(contentWidth, contentHeight) => {
								if (this.state.isNewMsg || this.state.nowPageNum == 1) {
									...
									this._scrollView.scrollTo({
										x: 0,
										y: contentHeight - scrollViewHeight - (height / 10),
										animated: false
									})
								}
								scrollViewHeight = contentHeight;
							...
							<View>
								{
									this.state.data.map((item, index) => {
										...
										if (body.basic.type == "privateChat") {
											time = typeof body.content.sendTime == "string" ? ToolUtil.strToStemp(body.content.sendTime) : body.content.sendTime;
										} else {
											time = item.timestamp;
										}
										if (tempTime != 0 && time && time - tempTime > 180000) {
											...
											console.log('1');
											views.push(this.timeView(new Date().getTime(), time));
										}
										console.log('===');
										tempTime = time;
										let sendType = '';
										...
										if (this.state.backPage == 'Message') {
											//判断消息是不是自己发的
											if (sendType == 'to') {
												views.push(this.myselfView('self', body, index, item));
											} else {
												views.push(this.myselfView('other', body, index, item));
											}
										} else {
											//群聊
											if (this.state.basic.jidNode == item.body.basic.userId) {
	...
	renderOptionPanel = () => {
		if(this.state.panelMark){
			// this._scrollView.scrollToIndex({animated:true,index:this.state.data.length-1,viewPosition: 0});
			return (
				<View style={{marginBottom: Platform.OS == 'ios' ? (DeviceInfo.getModel() == 'iPhone X' ? 34 : 0) : 0}}><BottomPanel
	...
	getFriendNowID = (info, item) => {
		...
		let tempBody = typeof item.body == 'string' ? JSON.parse(item.body) : item.body;
		let tempContent = typeof item.body == 'string' ? JSON.parse(item.body).content : item.body.content;
		...
		if (this.state.tempMsgType == 2) {
			...
			FetchUtil.netUtil(Path.redirectMsg, reqBody, 'POST', this.props.navigation, {
				...
				if (res.code.toString() == '200' && res.status) {
					...
					this.refs.toast.show('转发失败,请重试', DURATION.LENGTH_SHORT);
				}
			})
		}
		if (info.ttype == 1) {
		...
		let tempMsgBody = {
			...
			// "messageType": tempBody.messageType,
			"basic": info.ttype == 1 ? {
				"toId": info.jid_node,
				"type": "privateChat",
				"fromId": this.state.basic.jidNode,
