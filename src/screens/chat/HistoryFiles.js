...
export default class HistoryFiles extends Component {
				...
				<View style={[styles.fileBox, index === 0 ? {borderTopColor: 'transparent'} : null]}>
					...
					<View style={styles.fileBottom}>
						...
						<TouchableOpacity style={styles.fileInfor} onPress={() => {
							...
							<Text
								style={styles.fileSize}>{nowType + '  (' + ToolUtil.getFileSize(item.filesize) + ')'}</Text>
						</TouchableOpacity>
					</View>
				</View>
			</View>
	...
	_useOtherOpenFile = (file) => {
		console.log(Path.baseImageUrl + '?uuId=' + this.state.uuid + '&ticket=' + this.state.ticket + '&userId=' + this.state.basic.userId + '&fileId=' + file.id + '&fileName=' + encodeURI(encodeURI(file.fileName)));
		OpenFile.openDocBinaryinUrl([{
