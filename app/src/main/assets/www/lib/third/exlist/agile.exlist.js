...
(function($) {
	...
	function _controller(getli, option) {
		...
		function _init_dom() {
            ...
            $selector_li.off('swiperight').on('swiperight', swiperight);
		}
		...
		function swipeleft(e) {
			...
			$selector_li.parent().find('li').children(':not(.swipe_option)').off(A.options.clickEvent, fallback_swipeleft).on(A.options.clickEvent, fallback_swipeleft);

			//_li_element.off('swiperight', fallback_swipeleft).on('swiperight', fallback_swipeleft);

			_li_element.children(':not(.swipe_option)').off(A.options.clickEvent).on(A.options.clickEvent, function() {
				_li_element.animate({
					left: '0px'
				}, 100);
				_li_element.children(':not(.swipe_option)').off(A.options.clickEvent);
				return false;
			});

			_li_element.children('.swipe_option').off(A.options.clickEvent).on(A.options.clickEvent, function(e) {
		...
		function _showLeft() {
			...
			$selector_li.parent().find('li').children(':not(.swipe_option)').off('click', fallback_swipeleft);
	...
	ExList.liController = function($selector, opt) {
		return (function($selector, opt) {
			var option = {
				...
				rightContent: '<div style="width:100%;height:100%;background-color: #FF2D2D;text-align: center;line-height: 69px;color: #FFFFFF;">删除</div>'
