...
// var listCountMark;
refreshInit();
$('#toExpressReadedList_section').on('sectionshow', function () {
...
function refreshList(data) {
	...
	if (data.recordList != null && data.recordList.length > 0) {
		...
		for (var i = 0; i < data.recordList.length; i++) {
			...
			htmlStr += '<li>';
			htmlStr += '<a class="justify-content" onclick="showSignDialog(\'' + item.sendId + '\',\'' + item.id + '\')" href="javascript:;">';
...
function showSignDialog(recordId, id) {
	fhtp = "2";
	A.Controller.section('toExptessReadedDetail_section.html?id=' + id);
}
...
//待签详情处理js
var scroll;
$('#toExptessReadedDetail_article').on('scrollInit', function () {
	scroll = A.Scroll(this);//已经初始化后不会重新初始化，但是可以得到滚动对象
});
$('#toExptessReadedDetail_section').on('sectionshow', function () {
...
$('#signDetail-section').on('sectionhide', function () {
	$('#attachment').empty();
	$('.details').find('[name]').each(function (i, item) {
		$(item).empty();
	})
...
function backToSignList() {
	A.Controller.section('toExpressReadedList_section.html?fhtp=' + fhtp);
	A.Controller.page('#' + type);
}
