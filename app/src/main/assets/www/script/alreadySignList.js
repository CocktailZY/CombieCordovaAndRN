...
function refreshList(data) {
	...
	if (data.recordList != null && data.recordList.length > 0) {
		...
		for (var i = 0; i < data.recordList.length; i++) {
			...
			htmlStr += '<small class="right font12">' + item.sendStartTime + '</small>';
			htmlStr += '   </span>';
			htmlStr += '   </p>';
			htmlStr += '   </a>';
			htmlStr += '   </li>';
...
function showSignDialog(recordId, id) {
	fhtp = "2";
	A.Controller.section('toAlreadySignDetail-section.html?id=' + recordId);
	// getDataByAjax({
...
//待签详情处理js
var scroll;
$('#toAlreadySignDetail-article').on('scrollInit', function () {
	scroll = A.Scroll(this);//已经初始化后不会重新初始化，但是可以得到滚动对象
});
$('#toAlreadySignDetail-section').on('sectionshow', function () {
...
$('#toAlreadySignDetail-section').on('sectionhide', function () {
	$('#attachment').empty();
	$('.details').find('[name]').each(function (i, item) {
		$(item).empty();
	})
