...
function refreshInit() {
	...
	$('#reportManage').on('refreshInit', function () {
		refresh3 = A.Refresh('#reportManage');
		refresh3.setConfig({
			pullDownOpts: {
				normalLabel: '下拉刷新',
				releaseLabel: '释放刷新',
	...
	$('#consultManage').on('refreshInit', function () {
		refresh4 = A.Refresh('#consultManage');
		refresh4.setConfig({
			pullDownOpts: {
				normalLabel: '下拉刷新',
				releaseLabel: '释放刷新',
...
//刷新待签列表
$('#toReadedList_section').on('sectionshow', function () {
	...
	if (data.recordList != null && data.recordList.length > 0) {
		...
		for (var i = 0; i < data.recordList.length; i++) {
			...
			/*htmlStr += '<li><a class="justify-content" onclick="showSignDialog(\''+item.recordid+'\',\''+item.id+'\')"  href="javascript:;">'
				...
				+'</li>';*/
			htmlStr += '<li>';
			htmlStr += '<a class="justify-content" onclick="showSignDialog(\'' + item.sendId + '\',\'' + (item.id ? item.id : item.recordid) + '\')" href="javascript:;">';
		...
		A.Scroll('#reportManage').scrollTo(0, 0, 0, 0);
		A.Scroll('#consultManage').scrollTo(0, 0, 0, 0);
		A.Scroll('#receiptManagement').scrollTo(0, 0, 0, 0);
		A.Scroll('#docManage').scrollTo(0, 0, 0, 0);
	} else {
		//A.showToast('暂无数据~');
...
function showSignDialog(recordId, id) {
	fhtp = "2";
	A.Controller.section('toReadedSignDetail-section.html?id=' + id);
	// getDataByAjax({
...
//待签详情处理js
var scroll;
$('#toReadedSignDetail-article').on('scrollInit', function () {
	scroll = A.Scroll(this);//已经初始化后不会重新初始化，但是可以得到滚动对象
});
$('#toReadedSignDetail-section').on('sectionshow', function () {
			...
			getDataByAjax({
					...
					if (data != null && data.length > 0) {
							...
							for (var i = 0; i < data.length; i++) {
									attaches += '<p style=\"color: #278EEE;word-wrap:break-word;\" onclick="readPdf(\'' + data[i].attachId + '\')">' + data[i].attachName + '</p>';
							}
...
$('#toReadedSignDetail-section').on('sectionhide', function () {
	$('#attachment').empty();
...
function backToSignList() {
	isRefresh = false;
	A.Controller.section('toReadedList_section.html?fhtp=' + fhtp);
	// A.Controller.page('#'+type);
}
