...
// var listCountMark;
refreshInit();
$('#express_section').on('sectionshow', function () {
...
function refreshList(data) {
	...
	if (data.recordList != null && data.recordList.length > 0) {
		...
		for (var i = 0; i < data.recordList.length; i++) {
			...
			htmlStr += '<li><a class="justify-content" href="expressDetail.html?id=' + item.recordid + '&filetypeid=' + item.fileTypeId + '&workitemid=' + item.workitemid + '&listType=' + type + '&fhtp=' + fhtp + '">'//'&attr1='+item.attr1+
...
function loadMoreData(data) {
	...
	if (result == undefined) {
		...
		if (data.recordList != null && data.recordList.length > 0) {
			for (var i = 0; i < data.recordList.length; i++) {
				var item = data.recordList[i];
				htmlStr += '<li><a class="justify-content" href="expressDetail.html?id=' + item.recordid + '&filetypeid=' + item.fileTypeId + '&workitemid=' + item.workitemid + '&listType=' + type + '&fhtp=' + fhtp + '">'
					+ '<p><i class="bg-warn radiusround size8"></i><span class="lrmargin8 text-ellipsis-black" style="width: 85%;">' + item['title'] + '</span></p>'
					+ '<p class="list-font ml16">'
					+ '<span class="text-ellipsis-black" style="width: 70%;color: #aaaaaa;">'
					+ item['wfleveName']
