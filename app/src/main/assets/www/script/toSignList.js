	...
	if (data.recordList != null && data.recordList.length > 0) {
		...
		for (var i = 0; i < data.recordList.length; i++) {
			...
			/*htmlStr += '<li><a class="justify-content" onclick="showSignDialog(\''+item.recordid+'\',\''+item.id+'\')"  href="javascript:;">'
				...
				+'</li>';*/
			htmlStr += '<li class="signItem" id="' + item.recordid + '" attrRecordid="' + item.recordid + '" attrId="' + item.id + '" >';
...
function loadMoreData(data) {
	...
	if (data.recordList != null && data.recordList.length > 0) {
		...
		for (var i = 0; i < data.recordList.length; i++) {
			...
			htmlStr += ' <span class="text-ellipsis-black colorAaa" style="width: 70%;">';
			htmlStr += Util.getItem('userFullName');
			htmlStr += '<small class="right">' + (item.sendStartTime ? item.sendStartTime : item.registDate) + '</small>';
			htmlStr += '   </span>';
			htmlStr += '   </p>';
			htmlStr += '   </a>';
			htmlStr += '   </li>';
		...
		if (data.flag == '0') {
			...
			A.confirm('提示', '确定签收吗？', function (text) {
				getDataByAjax({
					...
					if (data.flag == '1') {
						initBackKeyDown(1);
						A.Controller.section('toSignDetail-section.html?id=' + recordId);
					}
				});
			}, function () {
			});
