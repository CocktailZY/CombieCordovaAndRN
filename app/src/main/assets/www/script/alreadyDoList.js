var refresh1, refresh2, refresh3, refresh4, refresh5, refresh6, pageNum = 1, totalPage, type = 'reportManage';
...
function refreshList(data) {
	...
	if (data.recordList != null && data.recordList.length > 0) {
		...
		for (var i = 0; i < data.recordList.length; i++) {
			...
			htmlStr += '<li><a class="justify-content" href="alreadyDetail.html?id=' + item.recordid + '&filetypeid=' + item.fileTypeId + '&workitemid=' + item.workitemid + '&listType=' + type + '&fhtp=' + fhtp + '">'//'&attr1='+item.attr1+
				+ '<p><i class="bg-warn radiusround size8"></i><span class="lrmargin8 text-ellipsis-black" style="width: 85%;">' + item['title'] + '</span></p>'
...
function loadMoreData(data) {
	...
	if (data.result == undefined) {
		...
		if (data.recordList != null && data.recordList.length > 0) {
			for (var i = 0; i < data.recordList.length; i++) {
				var item = data.recordList[i];
				htmlStr += '<li><a class="justify-content" href="alreadyDetail.html?id=' + item.recordid + '&filetypeid=' + item.fileTypeId + '&workitemid=' + item.workitemid + '&listType=' + type + '&fhtp=' + fhtp + '">'
					+ '<p><i class="bg-warn radiusround size8"></i><span class="lrmargin8 text-ellipsis-black" style="width: 85%;">' + item['title'] + '</span></p>'
					+ '<p class="list-font ml16">'
					+ '<span class="text-ellipsis-black" style="width: 70%;color: #aaaaaa;">'
					+ '<small class="right font12">' + item['readTime'] + '</small>'
