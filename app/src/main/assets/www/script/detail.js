...
function initDetail(detailId, type, detailType) {
	...
	// var url = "ConsultManage/view";
	// if(type == 'dodoc' || type == 'doreport') {
	//     url = "archiveReport/viewdoc";
	// }
...
function backToList() {
	...
	window.location.href = 'toDoList.html?type=' + type + '&fhtp=' + fhtp;
}
...
function getSelected() {
	...
	//console.log('_staticFlowObj.node[0].nodes.length');
	//console.log(_staticFlowObj.node[0].nodes.length);
	for (var dept = 0; dept < _staticFlowObj.node[0].nodes.length; dept++) {
		...
		for (var per = 0; per < _tree.selectedNodes.length; per++) {
			var tempPer = _tree.selectedNodes[per];
			for (var ids = 0; ids < tempDept.nodes.length; ids++) {
				var tempid = tempDept.nodes[ids];
				if (tempid.id == tempPer.id) {
					_choosePerObj.push(tempid);
...
var ooid = "";
//点击提交意见
$('#opiniona').click(function () {
	// $('#opiniona').attr('disable',true);//禁用按钮
	opthtml = [];
	$('#yxry').empty();
	$('#dxry').empty();
	idea = $('#description').val();
	sysid = Util.getItem('sysid');//$('#sysid').text();
...
//办结并提交
// $('#opiniona2').click(function () {
//     $('#opiniona2').attr('disable', true);//禁用按钮
//     idea = $('#description').val();
//     getDataByAjax({
...
//             window.location.href = 'toDoList.html?type=' + type;
//         } else {
//             A.showToast("操作失败！");
//         }
//         $('#opiniona2').attr('disable', flase);//启用按钮
//     }, null);
// });
//办结
$('#opiniona3').click(function () {
	$('#opiniona3').attr('disable', true);//禁用按钮
	idea = $('#description').val();
	getDataByAjax({
		//url:'http://192.168.81.6:8083/oa/MobileFlow/endflow',
		url: base + 'MobileFlow/endflow',
		data: {
			recordid: detailId,
			type: type,
		...
		$('#opiniona3').attr('disable', false);//启用按钮
	}, null);
});
//点击提交意见
$('#opiniona_docm').click(function () {
	// $('#opiniona').attr('disable',true);//禁用按钮
	opthtml = [];
	$('#yxry').empty();
	$('#dxry').empty();
	idea = $('#description_docm').val();
	sysid = Util.getItem('sysid');//$('#sysid').text();
	title = $('#title_docm').html();
	getDataByAjax({
		url: base + 'MobileFlow/toFlow',//获取路由
		data: {
			attr: '5',
		...
		if (data.result == undefined) {
			...
			if (data.flag == "selectdept") {//多个路由
				...
				if (receiveName == "") {
					if (opName == "") {
						A.showToast("发送成功");
						$("#sugBox_docm").attr("style", "display: none");
						$("#sugTextBox_docm").attr("style", "display:none");
						$("#btnsBox_docm").attr("style", "display:none");
					} else {
						A.showToast("结束本岗工作");
						$("#sugBox_docm").attr("style", "display: none");
						$("#sugTextBox_docm").attr("style", "display:none");
...
$('#opiniona3_docm').click(function () {
	...
	getDataByAjax({
		...
		$('#opiniona3_docm').attr('disable', false);//启用按钮
	}, null);
});
$('#opiniona_bj').click(function () {
	$('#opiniona_bj').attr('disable', true);//禁用按钮
	idea = $('#description').val();
	getDataByAjax({
		//url:"http://192.168.81.6:8083/oa/MobileFlow/endflow",
		url: base + 'MobileFlow/endflow',
...
function updatary(oid, luType) {
	...
	if (luType != 'single') {
		...
		getDataByAjax({
			...
			//     }
			// }
			// var fwid = data.node[0].nodes[0].id.replace("dept_", "");
			// for (var i = 0; i < opthtml.length; i++) {
			//     if (opthtml[i].indexOf("opt_" + fwid) >= 0) {
			//         $('#dxry').append(opthtml[i]);
			//     }
			// }
...
function testJPush(ids) {
	...
	//         // alias:[{deptid1:'id1,id2,id3'},{deptid2:'id1,id2,id3'},{deptid3:'id1,id2,id3'}]
	//     }
	// }, function (data) {
	//     // Util.redirectLogin(data.result);
	//     if (data.status == '-1') {
...
// $('#sndiv').on("touchstart",function(event){
//     $('#sndiv').hasClass('active')?($('#sndiv').removeClass('active')&&$('#sninput').val('')):($('#sndiv').addClass('active')&&$('#sninput').val('1'));
// })
function initBackKeyDown(type) {
	if (type == 1) {
		...
		document.removeEventListener("backbutton", backToDetail, false); // 注销返回键
		document.addEventListener("backbutton", backToChoose, false);//绑定首页
	} else {
		document.removeEventListener("backbutton", onBackKeyDown, false); // 注销返回键
		document.removeEventListener("backbutton", exitApp, false); // 注销返回键
		document.removeEventListener("backbutton", backToList, false); // 注销返回键
